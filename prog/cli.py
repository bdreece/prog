...
@click.command(
    ...
    help=
    'A command line utility for centralizing scripted shell commands via a configurable JSON or YAML file'
)
@click.option('-f',
              '--file',
              type=click.Path(),
...
def cli(ctx, json, yml, verbose, commands):
    ...
    with open(ctx.obj['path'], 'rt') as f:
        if ctx.obj['path'][-4:] == 'json':
            ...
            click.echo('[ERROR] ' + ctx.obj['path'][-4:] +
